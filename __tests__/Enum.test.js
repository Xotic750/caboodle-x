import {Enum} from '../src/caboodle-x';
...
describe('enum', () => {
  ...
  beforeEach(() => {
    ...
    subject = null;
    subjectName = 'subject';
    subjectProps = ['RED', 'YELLOW', {name: 'BLUE', value: 10}, 'PINK', {name: 'BLACK', value: 1}, {name: 'GREY', value: Object}];
  });
  ...
  it('enum has symIt', () => {
    expect(typeof Enum.SYMBOL_ITERATOR).toBe(hasIteratorSupport ? 'symbol' : 'string');
  });
  ...
  it('subject should have working forEach', () => {
    ...
    subject = Enum.create(subjectName, subjectProps);
    let index = 0;
    subject.forEach(function iteratee(enumMember) {
