import {isDate} from '../src/caboodle-x';
...
describe('isDate', () => {
  it('not Dates', () => {
    ...
    expect(isDate(new RegExp('a', 'g'))).toBe(false, 'regex object is not Date');
  });
  ...
  itHasSymbols('@@toStringTag', () => {
    ...
    expect(isDate(fakeDate)).toBe(false, 'fake Date with @@toStringTag "Date" is not Date');
  });
